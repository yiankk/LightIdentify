...
# 原始图片路径
original_path = 'E:\\pythonProject\\CV_EX\\Light_Identify\\Pictures\\light\\'

# 批处理保存路径
# canny框选路径
pro1_path = 'E:\\pythonProject\\CV_EX\\Light_Identify\\Pictures\\canny\\'
# 黑色筛选框选路径
pro2_path = 'E:\\pythonProject\\CV_EX\\Light_Identify\\Pictures\\black\\'
# canny和黑色框合成路径
pro3_path = 'E:\\pythonProject\\CV_EX\\Light_Identify\\Pictures\\canny_black\\'
# 颜色框选路径
pro4_path = 'E:\\pythonProject\\CV_EX\\Light_Identify\\Pictures\\hsv\\'
# 最终成品路径
pro5_path = 'E:\\pythonProject\\CV_EX\\Light_Identify\\Pictures\\final\\'
# 视频路径
vedio_path = 'E:\\pythonProject\\CV_EX\\Light_Identify\\vedio\\'

start_directory = r'E:\pythonProject\CV_EX\Light_Identify'
...
def img_processing2(img, width, high):
    image = cv.GaussianBlur(img, (7, 7), 1)  # 高斯滤波
    hsv = cv.cvtColor(image, cv.COLOR_BGR2HSV)
    inRange_hsv = cv.inRange(hsv, np.array([0, 0, 0]), np.array([180, 255, 85]))
    kernel = np.ones((3, 3), np.uint8)
...
def hsv_cope(image, width, high):
    color_dist = {
        'green': {'Lower': np.array([45, 125, 125]), 'Upper': np.array([90, 255, 255])},
        'red': {'Lower': np.array([0, 145, 146]), 'Upper': np.array([14, 255, 255])},
        'deep_red': {'Lower': np.array([160, 143, 146]), 'Upper': np.array([180, 255, 255])}
    }
    image = cv.GaussianBlur(image, (7, 7), 1)  # 高斯滤波
    hsv = cv.cvtColor(image, cv.COLOR_BGR2HSV)
...
def compare(list_1, record_1, list_2, record_2):
        ...
        for i in range(record_1):
            for j in range(record_2):
                if (abs(2 * list_1[i][0] + list_1[i][2] - 2 * list_2[j][0] - list_2[j][2]) < list_1[i][2] + list_2[j][2]
                        and abs(2 * list_1[i][1] + list_1[i][3] - 2 * list_2[j][1] - list_2[j][3]) < list_1[i][3] +
                        list_2[j][3]):
                    list_1[i] = 0
                    count += 1
                    break
    ...
    for p in range(x):
        if list_new[p] != 0:
            ...
            if (list_new[p][2] / list_new[p][3] < 0.2 or 0.55 < list_new[p][2] / list_new[p][3] < 0.8
                    or 1.3 < list_new[p][2] / list_new[p][3] < 2 or list_new[p][2] / list_new[p][3] > 3.75):
                list_new[p] = 0
                count += 1
...
class LOGIN(QMainWindow, Ui_LOGIN):
    ...
    def display(self):
        if USER_PWD.get(self.username_line.text()) == self.password_line.text():
...
class MENU(QMainWindow, Ui_MENU):
    def __init__(self, parent=None):
        ...
        self.Button3.clicked.connect(self.num2)
        self.Button4.clicked.connect(self.num3)
    ...
    def num1(self):
        QMessageBox.information(self, 'Information', '图片处理中,请等待!')
        for k in range(len(original_list)):
            ...
            for i in range(hsv_record_red):
                cv.rectangle(img1, (hsv_list_red[i][0], hsv_list_red[i][1]),
                             (hsv_list_red[i][0] + hsv_list_red[i][2], hsv_list_red[i][1] + hsv_list_red[i][3]), (0, 255, 0), 2)
            for i in range(hsv_record_green):
                cv.rectangle(img1, (hsv_list_green[i][0], hsv_list_green[i][1]),
                             (hsv_list_green[i][0] + hsv_list_green[i][2], hsv_list_green[i][1] + hsv_list_green[i][3]), (0, 255, 0), 2)
        ...
        QMessageBox.information(self, 'Information', '图片处理完成!')

    def num2(self):
        # number1 = input("请输入修改图片名称数字: ")
        img = cv.imread(original_path + f'{self.lineEdit1.text()}.jpg')
        # img = cv.resize(img, (1024, 760))
        img1 = cv.imread(original_path + f'{self.lineEdit1.text()}.jpg')
        img2 = cv.imread(original_path + f'{self.lineEdit1.text()}.jpg')
        img3 = cv.imread(original_path + f'{self.lineEdit1.text()}.jpg')
        img4 = cv.imread(original_path + f'{self.lineEdit1.text()}.jpg')
        img5 = cv.imread(original_path + f'{self.lineEdit1.text()}.jpg')
        ...
        hsv_list_green, hsv_record_green, hsv_list_red, hsv_record_red = hsv_cope(img3, width, high)
        for i in range(hsv_record_red):
            cv.rectangle(img3, (hsv_list_red[i][0], hsv_list_red[i][1]),
                         (hsv_list_red[i][0] + hsv_list_red[i][2], hsv_list_red[i][1] + hsv_list_red[i][3]),
                         (0, 255, 0), 2)
        for i in range(hsv_record_green):
            cv.rectangle(img3, (hsv_list_green[i][0], hsv_list_green[i][1]),
        ...
        list_2, record_2 = img_processing2(img1, width, high)
        for i in range(record_2):
            cv.rectangle(img1, (list_2[i][0], list_2[i][1]),
                         (list_2[i][0] + list_2[i][2], list_2[i][1] + list_2[i][3]), (0, 255, 0), 2)
        list_new, record_new = compare(list_1, record_1, list_2, record_2)
        for i in range(record_new):
            cv.rectangle(img2, (list_new[i][0], list_new[i][1]),
                         (list_new[i][0] + list_new[i][2], list_new[i][1] + list_new[i][3]), (0, 255, 0), 2)
        rect(img4, record_new, list_new, hsv_record_green, hsv_list_green, hsv_record_red, hsv_list_red)

        # cv.imshow('canny', img)
        # cv.imshow('black+rectangle', img1)
        # cv.imshow('canny+black+rectangle', img2)
        # cv.imshow('red+green', img3)
        cv.imshow('finally', img4)
        cv.imshow('origin', img5)
        cv.waitKey(0)
        cv.destroyAllWindows()

    def num3(self):
        cap = cv.VideoCapture(vedio_path + f'{self.lineEdit2.text()}.mp4')
        self.lineEdit2.clear()
        QMessageBox.information(self, 'Information', '视频处理中,请等待!')
        fps = 20
        size = (1280, 720)
        ...
        if not cap.isOpened():
            print("视频无法打开")
            QMessageBox.information(self, 'Information', '视频无法打开')
            exit()
        ...
        cap = cv.VideoCapture('cope.avi')
        while cap.isOpened():
            ret, frame = cap.read()
            if frame is None:
                break
            if ret:
                cv.imshow("cope", frame)
