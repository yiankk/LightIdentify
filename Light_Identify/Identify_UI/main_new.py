...
# 原始图片路径
original_path = 'G:\\Light_Identify\\light\\'

# 批处理保存路径
# 灰度,降噪图路径
pro1_path = 'G:\\Light_Identify\\light_cope1\\'
# 轮廓绘制图路径
pro2_path = 'G:\\Light_Identify\\light_cope2\\'
# 矩形绘制图路径
pro3_path = 'G:\\Light_Identify\\light_cope3\\'
...
def img_processing2(img, width, high):
    ...
    record_2 = 0
    for obj in contours:
        area = cv.contourArea(obj)  # 计算轮廓面积
        if area > 0.0001 * width * high:
            # cv.drawContours(dst1, obj, -1, (255, 0, 0), 4)
            perimeter = cv.arcLength(obj, True)  # 计算周长
            approx = cv.approxPolyDP(obj, 0.02 * perimeter, True)  # 轮廓点坐标
            CornerNum = len(approx)  # 点的数量
...
if __name__ == '__main__':
    ...
    if number == '1':
        for k in range(len(original_list)):
            ...
            for i in range(record_new):
                cv.rectangle(img1, (list_new[i][0], list_new[i][1]),
            ...
            rect(img2, record_new, list_new, hsv_record_green, hsv_list_green, hsv_record_red, hsv_list_red)
    ...
    if number == '3':
        cap = cv.VideoCapture("E:\\pythonProject\\CV_EX\\Light_Identify\\vedio\\light1.mp4")
        fps = 20
        size = (1280, 720)
